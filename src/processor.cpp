...
void Processor::CalculateDeltaTime(vector<int>& first_cpu_jiffies, vector<int>& second_cpu_jiffies) {
  delta_time_.clear();
  for (int i = 0; i < 3; ++i) {
...
long Processor::getTotalJiffies() { return current_cpu_jiffies_.at(CPUTime::kTotalTime_); }